...
// src/sections/CommercialInfoSection.tsx
import React, { useEffect, useRef, useState } from "react";
import { motion } from "framer-motion";
import gsap from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";
...
export default function CommercialInfoSection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      source: "CommercialInfoSection",
      full_summary: `Name: ${form.name}\nMobile: ${form.mobile}\nEmail: ${form.email}\nMessage: ${form.message}`,
    };
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.error("EmailJS send failed:", res.error);
                  ...
                  <form onSubmit={handleSubmit} className="space-y-3">
                    ...
                    <div className="pt-4">
                      <motion.button
                        type="submit"
                        whileTap={{ scale: 0.98 }}
                        className="w-full rounded-full py-3 text-[15px] font-poppins font-light tracking-[0.05em] bg-gradient-to-b from-[#e30715] to-[#e30715] hover:from-[#ff1a23] hover:to-[#e30715] transition text-white shadow-md disabled:opacity-60"
                        disabled={sending}
                      >
